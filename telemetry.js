...
function renderTelemetry(logs, selectedWeek = "all") {
    ...
    const THEME = {
        ...
        getPerformanceColor: function(hrs) {
            ...
            if (hrs >= 10) return this.highest; // Elite
            if (hrs >= 8)  return this.mid;     // Solid
            return this.low;                    // Survival/Low
        }
    };
    ...
    // (Core Hours / Total Hours including Personal) * 100
    const totalWorkHours = logs.reduce((sum, r) => sum + r.hours, 0);
    const totalPersonal = logs.reduce((sum, r) => sum + (r.personalHours || 0), 0);
    const efficiency = totalWorkHours > 0 
        ? (totalWorkHours / (totalWorkHours + totalPersonal)) * 100 
        : 0;

    // 2. Momentum Calculation
    // Comparing Last 7 Days Avg vs Overall Average Pace
    const momentum = overallPace > 0 
    ? ((last7DayAvg - overallPace) / overallPace) * 100 
    : 0;
    ...
    if (momentumEl) {
        momentumEl.style.color = momentum >= 0 ? THEME.mid : THEME.low;
    }
...
function renderIdentityChart(logs, colors) {
    ...
    charts.identity = new Chart(ctx, {
        ...
        options: {
            ...
            maintainAspectRatio: false,
            scales: {
                y: { min: 0, max: 5, grid: { color: colors.grid }, ticks: { color: colors.text } },
